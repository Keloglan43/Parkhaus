...
for (let i = 0; i < parkhaus.anzEtagen; i++) {
    ...
    for (let k = 0; k < parkhaus.plaetzeProEtage; k++) {
        ...
        if (k < 7 && i == 0){ platz.geschlecht = "Weiblich"}
...
let etagenZahl:number = 0

function einparken(kzeichen:string) {
    let geparkt = false;
    while (!geparkt) {
        for (let i = 0; i < parkhaus.anzEtagen; i++){
...
function einparkenMitG(kzeichen:string, geschlecht:string) {
    let geparkt = false;
    while (!geparkt) {
        for (let i = 0; i < parkhaus.anzEtagen; i++){
            for (let j = 0; j < parkhaus.etagen[i].anzPlaetze; j++) {
                if (parkhaus.etagen[i].plaetze[j].frei == true && parkhaus.etagen[i].plaetze[j].geschlecht == geschlecht) {
...
function ZahlGen(text:string, x:number){
    for (let i = 0; i < x; i++){

        let rd:string = Math.floor(Math.random() * 10).toFixed(0)
        text = text + rd;
    }
    console.log(text)
}
...
function ausparkenMitKZ (x:string){
    let kzGefunden = true;
    for(let i = 0; i < parkhaus.anzEtagen; i++){
        for (let j = 0; j < parkhaus.etagen[i].anzPlaetze; j++){
...
ausparkenMitKZ ("NE NE 1") // True
ausparkenMitKZ("NE NE 1234") // False
function Info (){
    parkhaus
}
